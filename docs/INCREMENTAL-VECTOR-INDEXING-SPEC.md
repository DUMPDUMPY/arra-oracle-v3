...
- 2026-07-16: the local `0041_incremental_vector_jobs` migration was first tested on a disposable DB copy, then applied to the live canonical DB after a timestamped pre-migration backup; `PRAGMA integrity_check = ok`. It is numbered `0042_incremental_vector_jobs` on this branch because upstream now owns `0041_fleet_log`.
